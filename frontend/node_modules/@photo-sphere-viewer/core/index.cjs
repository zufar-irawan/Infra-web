  ...
  if (adapter) {
    for (const [, p] of [["_", adapter], ...Object.entries(adapter)]) {
      if (p.prototype instanceof AbstractAdapter) {
        checkVersion(p.id, p.VERSION, "5.14.0");
        return p;
      }
    }
  }
...
_EquirectangularAdapter.VERSION = "5.14.0";
...
_DualFisheyeAdapter.VERSION = "5.14.0";
        ...
        switch (transition.effect) {
          ...
          case "white":
            if (props.opacity < 0.5) {
              this.renderer.toneMappingExposure = transition.effect === "black" ? import_three11.MathUtils.mapLinear(props.opacity, 0, 0.5, 1, 0) : import_three11.MathUtils.mapLinear(props.opacity, 0, 0.5, 1, 5);
            } else {
              this.renderer.toneMappingExposure = transition.effect === "black" ? import_three11.MathUtils.mapLinear(props.opacity, 0.5, 1, 0, 1) : import_three11.MathUtils.mapLinear(props.opacity, 0.5, 1, 5, 1);
...
var AbortableImageLoader = class extends import_three12.ImageLoader {
  constructor() {
    super(...arguments);
...
var TextureLoader = class extends AbstractService {
  ...
  constructor(viewer) {
    super(viewer);
    this.fileLoader = new import_three13.FileLoader();
