  ...
  useEffect(() => {
    const fetchKegiatan = async () => {
      try {
        ...
        if (json.success && Array.isArray(json.data)) setKegiatan(json.data);
      } catch (err) {
      ...
      <section className="w-full py-4 bg-white">
        <div className="container mx-auto px-4">
          <nav className="flex items-center text-sm font-medium space-x-2">
            ...
            <Link href="/informasi/kegiatan" className="text-[#FE4D01] hover:underline">
      ...
      <section className="py-20 bg-[#243771] flex flex-col items-center justify-center">
        <div className="container mx-auto px-6 md:px-12 lg:px-20 text-center">
          <h2 className="text-3xl md:text-4xl font-bold mb-3 text-white">
            {lang === "id" ? "Agenda Kegiatan Terbaru" : "Latest Activity Schedule"}
          </h2>
          ...
          {loading ? (
            <p className="text-gray-300 italic">Memuat kegiatan...</p>
          ) : kegiatan.length === 0 ? (
            <p className="text-gray-300 italic">
              {lang === "id" ? "Belum ada kegiatan." : "No events yet."}
            </p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 justify-items-center">
              {kegiatan.map((item) => (
                ...
                >
                  ...
                  <p className="text-sm text-gray-700 font-medium mt-auto">{item.place}</p>
                </div>
              ))}
            </div>
          )}
